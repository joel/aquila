...
describe Notifier do
  ...
  it 'should be set to be delivered from the email passed in' do
    expect(@email).to deliver_from ENV['APP_MAILER_FROM']
  end

  it 'should contain the user\'s message in the mail body' do
    expect(@email).to have_body_text(/VaultyPass Test Mail/)
  end
  ...
  context 'out of time' do
    ...
    it 'should have the correct subject' do
      expect(@email).to have_subject "[TEST] [VaultyPass] Test Mail #{Time.now.to_s(:number)}"
    end
  end
